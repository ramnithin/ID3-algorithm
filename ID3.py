...
def load_data(trainf, testf):
	Xtrain = pd.read_csv(trainf, header=None, delim_whitespace=True)
	...
	trainf_label = trainf.split('.')[0][:-4] + 'labs.csv'
	Ytrain = pd.read_csv(trainf_label, header=None, delim_whitespace=True)
...
pval = args['p']
Xtrain_name = args['f1']
Ytrain_name = args['f1'].split('.')[0][:-4]+ 'labs.csv' #labels filename will be the same as training file name but with _label at the end
print(Ytrain_name)
Xtest_name = args['f2']
